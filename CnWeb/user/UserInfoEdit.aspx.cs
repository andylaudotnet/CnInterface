...
namespace CnWeb.user
{
    public partial class UserInfoEdit : System.Web.UI.Page
    {
        ...
        private void LoadUserInfo()
        {
            ...
            string sql = "select * from Cn_InterfaceUser where username='" + username + "'";
        ...
        protected void btnregister1_Click(object sender, EventArgs e)
        {
            ...
            string conStr = System.Configuration.ConfigurationManager.AppSettings["ConnectionStr"].ToString();
            string sql = @"update Cn_InterfaceUser
set 
Remark=@Remark, 
UserQQ=@UserQQ,
RealName=@RealName,
...
where UserName=@UserName";
            ...
            using (SqlConnection conn = new SqlConnection(conStr))
            {
                ...
                if (ai > 0)
                {
                    Page.RegisterStartupScript("ok", "<script>alert('更新成功！');window.location.href='UserInfoEdit.aspx';</script>");
                }
                else
                {
        ...
        private void CheckNameIsExist(string name, string realname)
        {
            ...
            string conStr = System.Configuration.ConfigurationManager.AppSettings["ConnectionStr"].ToString();
            string sql = "select count(ID)  from Cn_InterfaceUser where Realname='" + Server.HtmlDecode(name.Trim()) + "'";
            int count = 0;
            using (SqlConnection conn = new SqlConnection(conStr))
            {
                conn.Open();
        ...
        protected void btnUploadImg_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                if (System.IO.Path.GetExtension(FileUploadImg.FileName) == ".jpg" || System.IO.Path.GetExtension(FileUploadImg.FileName) == ".JPG")
                {
                    ...
                    string conStr = System.Configuration.ConfigurationManager.AppSettings["ConnectionStr"].ToString();
                    string sql = "update Cn_InterfaceUser set ImageUrl='" + imgUserImg.ImageUrl + "' where username='" + Session["username"].ToString() + "'";
                    int value = 0;
                    using (SqlConnection conn = new SqlConnection(conStr))
                    {
                        conn.Open();
